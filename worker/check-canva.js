...
async function refreshMemberSnapshot(supabase, accountId, emails) {
  ...
  if (error) console.error('[member snapshots]', error.message);
}
...
async function findNextButton(page) {
  const patterns = [/^next$/i,/next page/i,/^berikutnya$/i,/^selanjutnya$/i,/halaman berikutnya/i];
  for (const pattern of patterns) {
    const byRole = page.getByRole("button", { name: pattern }).last();
    if (await byRole.count()) {
      try { if (await byRole.isVisible() && await byRole.isEnabled()) return byRole; } catch {}
    }
  }
  return null;
}
...
async function scanMemberEmails(page) {
  ...
  for (let p = 0; p < 40; p++) {
    ...
    if (!next) break;
    try { await next.click({ timeout: 2500 }); await sleep(1200); } catch { break; }
  }
  return [...found].sort();
}
...
async function tryRemoveMember(page, email) {
  ...
  const removePatterns = [/hapus dari tim/i,/keluarkan dari tim/i,/hapus anggota/i,/remove from team/i,/remove member/i,/remove user/i];
  for (const pattern of removePatterns) {
    ...
    try {
      ...
      await sleep(700);
      const confirms = [/^hapus$/i,/^keluarkan$/i,/^remove$/i,/^konfirmasi$/i,/^confirm$/i,/hapus dari tim/i,/keluarkan dari tim/i,/remove from team/i];
      for (const c of confirms) {
        const btn = page.getByRole("button", { name: c }).last();
        if (await btn.count()) { try { await btn.click({ timeout: 1400 }); } catch {} break; }
      }
      await sleep(1600);
...
async function processAccount(supabase, account) {
  ...
  try {
    ...
    const unauthorized = memberEmails.filter(email => !activeByEmail.has(email) && !protectedSet.has(email));
    ...
    if (account.auto_remove && !destructiveAllowed) {
      ...
      await updateAccountScan(supabase, account.id, { total: detected, unauthorized: unauthorizedCount, removed: 0, error });
...
Scan hanya mendeteksi ${detected} email, di bawah ambang aman ${minSafe}. Auto-remove DIBLOKIR untuk pemeriksaan ini.`);
    ...
    for (const email of unauthorized) {
      ...
      await audit(supabase, account.id, grant?.id, email, "unauthorized_detected", { reason, auto_remove: account.auto_remove });
      if (!account.auto_remove) continue;

      const result = await tryRemoveMember(page, email);
      await audit(supabase, account.id, grant?.id, email, result.ok ? "member_removed" : "remove_failed", result);
      if (result.ok) {
        removedCount += 1;
        if (grant) {
          const { error } = await supabase.from("canva_access").update({ status: "removed", removed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", grant.id);
          if (error) console.error("[mark removed]", error.message);
        }
      }
    }

    await updateAccountScan(supabase, account.id, { total: detected, unauthorized: unauthorizedCount, removed: removedCount, error: null });
    await audit(supabase, account.id, null, null, "scan_completed", { detected, active_access: activeByEmail.size, unauthorized: unauthorizedCount, removed: removedCount, auto_remove: account.auto_remove });
...
async function runOnce() {
  ...
  const { data: accounts, error } = await supabase.from("canva_accounts").select("*").eq("is_active", true).order("created_at");
  if (error) throw error;
  if (!accounts?.length) { console.log("Tidak ada akun Canva aktif."); return; }

  for (const account of accounts) {
    try { await processAccount(supabase, account); }
    catch (err) {
      console.error(`[${account.name}]`, err);
      await audit(supabase, account.id, null, null, "checker_error", { message: err.message });
      await updateAccountScan(supabase, account.id, { total: Number(account.last_scan_total || 0), unauthorized: 0, removed: 0, error: err.message });
